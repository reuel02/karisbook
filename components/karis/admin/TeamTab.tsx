...
const EMPTY_FORM: Omit<Professional, 'id'> = {
  name: '', role: '', initials: '', color: AVATAR_COLORS[0], services: [],
}

export default function TeamTab({ professionals, services, onAdd, onEdit, onDelete }: Props) {
  ...
  const openEdit = (p: Professional) => {
    setEditing(p)
    setForm({ name: p.name, role: p.role, initials: p.initials, color: p.color, services: [...p.services] })
    setSlideOpen(true)
  }
